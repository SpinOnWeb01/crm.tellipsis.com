...
import { GET_DID_FAIL, GET_DID_SUCCESS } from "../../redux/constants/destinationConstants";
...
function DID_TFN_number({ colorThem, selectedPortal }) {
  const state = useSelector((state) => state);
  const token = JSON.parse(localStorage.getItem("crm_token"));
  const dispatch = useDispatch();
  const user = JSON.parse(localStorage.getItem("crm_token"));
  ...
  useEffect(() => {
    ...
    dispatch({ type: GET_DID_FAIL, payload: "Error fetching data" });
    setLoader(false);
  }
  }, [radioValue, response, state.portal.selectedPortal]);
  useEffect(() => {
    if (state.portal.selectedPortal !== "crm") {
    dispatch(getAdminRedirectGroups(userId));
    }
  }, [userId, state.portal.selectedPortal]);
  useEffect(() => {
    if (state.portal.selectedPortal !== "crm") {
