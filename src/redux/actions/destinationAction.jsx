...
import {
  CREATE_DESTINATION_FAIL,
  CREATE_DESTINATION_REQUEST,
  CREATE_DESTINATION_SUCCESS,
  GET_DID_FAIL,
  GET_DID_REQUEST,
  GET_DID_SUCCESS,
  GET_REDIRECT_GROUPS_FAIL,
  GET_REDIRECT_GROUPS_REQUEST,
  GET_REDIRECT_GROUPS_SUCCESS,
  UPDATE_ADMIN_SUSPEND_DESTINATION_FAIL,
  UPDATE_ADMIN_SUSPEND_DESTINATION_REQUEST,
  UPDATE_ADMIN_SUSPEND_DESTINATION_SUCCESS,
...
export const getDid = (radioValue, setLoader) => async (dispatch, getState) => {
  try {
    const configData = getPortalConfig(getState());

    if (!configData) {
      // agar config null hai to request hi mat bhejo
      dispatch({
    ...
    const { apiBase, headers } = configData;
    dispatch({ type: GET_DID_REQUEST });
    let config = {
      method: "get",
      maxBodyLength: Infinity,
      url: `${apiBase}/didresource?status=${radioValue}`,
      headers: headers,
    };
    await axios
      .request(config)
...
export const getAdminRedirectGroups = (id) => async (dispatch, getState) => {
  try {
    const configData = getPortalConfig(getState());
    ...
    let config = {
      method: "get",
      maxBodyLength: Infinity,
      url: `${apiBase}/redirectgroups?user_id=${id}`,
      headers: headers,
    };
    await axios
      .request(config)
...
export const createDestination =
  (createdid, setOpen, setResponse) => async (dispatch, getState) => {
    try {
          const configData = getPortalConfig(getState());
...
export const updateDestination =
  (updateDid, setResponse, handleEditClose) => async (dispatch) => {
...
export const updateAssignment =
  (updateAssignment, setResponse, setEdit) => async (dispatch) => {
    const token = JSON.parse(localStorage.getItem("admin"));
    try {
      dispatch({ type: UPDATE_DESTINATION_REQUEST });
      const config = {
  ...
  (updateSuspendDid, setResponse) => async (dispatch) => {
    try {
      ...
      if (data?.status === 200) {
        ...
        setResponse(data);
        dispatch({
          type: UPDATE_ADMIN_SUSPEND_DESTINATION_SUCCESS,
          payload: data,
        });
      } else {
        toast.error(data?.message, {
