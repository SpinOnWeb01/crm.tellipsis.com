...
export const getHistory = (filterdata) => async (dispatch, getState) => {
  // Retrieve the token
...
export const getAddMinuteHistory = () => async (dispatch, getState) => {
  // Retrieve the token
  const { apiBase, headers } = getPortalConfig(getState());
  ...
  try {
    ...
    const config = {
      method: "get",
      maxBodyLength: Infinity,
      url: `${apiBase}/totaladdminutes`,
      headers: headers, // Use 'data' instead of 'body' to send payload
    };
